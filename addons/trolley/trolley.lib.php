...
function setDocumentEvent($addon_info)
{
	...
	if(!$addon_info->event_count)
	{
		$event_count = 1;
	}
	else
	{
		$event_count = $addon_info->event_count;
	}
	...
	$o_event_size = 300;
	if($addon_info->o_event_size)
	{
		$o_event_size = $addon_info->o_event_size;
	}
	Context::set('o_event_size', $o_event_size);

	$f_event_size = 120;
	if($addon_info->f_event_size)
	{
		$f_event_size = $addon_info->f_event_size;
	}
