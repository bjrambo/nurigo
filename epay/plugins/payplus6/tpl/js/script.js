...
function payplus_pay(join_form) {
	var paymethod = jQuery('input[name=payment_method]:checked','#fo_payplus').val();
	switch(paymethod) {
		case 'CC':
			jQuery('input[name=pay_method]','#fo_payplus').val('100000000000');
			break;
		case 'IB':
			jQuery('input[name=pay_method]','#fo_payplus').val('010000000000');
			break;
		case 'VA':
			jQuery('input[name=pay_method]','#fo_payplus').val('001000000000');
			break;
		case 'MP':
			jQuery('input[name=pay_method]','#fo_payplus').val('000010000000');
			break;
	}

	if (join_form) {
		copy_form(join_form, 'fo_payplus');
	}
	procFilter(document.getElementById('fo_payplus'), submit_review);
	return false;
}
