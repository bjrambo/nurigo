...
function completeInsertItem(ret_obj, response_tags, callback_func_args, fo_obj) {
    ...
    fo_obj.act.value = 'procNproductInsertItemFile';
...
function completeUpdateItem(ret_obj, response_tags, callback_func_args, fo_obj) {
	alert(ret_obj['message']);
    fo_obj.act.value = 'procNproductUpdateItemFile';
    fo_obj.submit();
}
...
function completeInsertDisplayItem(ret_obj) {	
	...
	exec_xml('nproduct', 'getNproductDisplayItems', params, completeGetFrontDisplayItems, responses);
}
...
function appendToDisplayStand(module_srl, item_srl) {
	...
	params['category_srl'] = selected_category_srl;
	exec_xml('nproduct', 'procNproductAdminInsertDisplayItem', params, completeInsertDisplayItem);
}
...
function delete_display_item(category_srl, item_srl) {
	...
	params['item_srl'] = item_srl;
	exec_xml('nproduct', 'procNproductAdminDeleteDisplayItem', params, function(ret_obj) {
		var params = new Array();
		var responses = ['error','message','data','category_srl'];
		params['category_srl'] = category_srl;
		exec_xml('nproduct', 'getNproductDisplayItems', params, completeGetFrontDisplayItems, responses);
	   	//alert(ret_obj['message']); location.href = current_url; 
	});
}
...
function append_category(module_srl) {
	...
	exec_xml('nproduct', 'procNproductAdminInsertDisplayCategory', params, function(ret_obj) { alert(ret_obj['message']); location.href = current_url; });
}
...
function delete_category(category_srl) {
	...
	var params = new Array();
	params['category_srl'] = category_srl;
	exec_xml('nproduct', 'procNproductAdminDeleteDisplayCategory', params, function(ret_obj) { alert(ret_obj['message']); location.href = current_url; });
}
		...
		success : function (r) {
			if (r.error === -1) {
				...
				for (i = 0; i < r.length; i++) {
					jQuery('<option value="' + r[i].attr.node_id + '">' + r[i].attr.node_name + '</option>').appendTo(target);
				}
			}
		}
	});
