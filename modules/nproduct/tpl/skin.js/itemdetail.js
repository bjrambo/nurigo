...
jQuery(function($) {
	$('#select_options').change(function() {
		...
		if (!$('#option_' + option_srl).length) {
			if (type == 'Y') {
				$('#selected_options').append('<tr id="option_' + option_srl + '" data-price="' + (price) + '"><td>' + title + str_price + '</td><td><input type="hidden" name="option_srls" value="' + option_srl + '" /><div><input type="text" name="quantities" class="quantity" value="1"' + 'id="quantity_'+ option_srl +'" style="width:40px; padding:6px" /><div class="iconUps" data-for="quantity_'+ option_srl +'"></div><div class="iconDowns" data-for="quantity_'+ option_srl +'"></div>' + xe.lang.each + '</td><td><span onclick="jQuery(this).parent().parent().remove(); printTotalPrice();" class="deleteItem">X</span></td><td><span>' + number_format(g_discounted_price + (price)) + '</span></td></tr>');
			} else {
				$('#selected_options').append('<tr id="option_' + option_srl + '" data-price="' + (g_discounted_price + (price)) + '"><td>' + title + str_price + '</td><td><input type="hidden" name="option_srls" value="' + option_srl + '" /><div><input type="text" name="quantities" class="quantity" value="1"' + 'id="quantity_'+ option_srl +'" style="width:40px; padding:6px" /><div class="iconUps" data-for="quantity_'+ option_srl +'"></div><div class="iconDowns" data-for="quantity_'+ option_srl +'"></div>' + xe.lang.each + '</div></td><td><span onclick="jQuery(this).parent().parent().remove(); printTotalPrice();" class="deleteItem">X</span></td><td><span>' + number_format(g_discounted_price + (price)) + '</span></td></tr>');
			}
		}
