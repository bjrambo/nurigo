...
class nproductAdminController extends nproduct
{
	...
	function checkColumnName($module_srl, $column_name)
	{
		...
		// check in db fields
		$oDB = &DB::getInstance();
		if($oDB->isColumnExists('nproduct_items', $column_name))
		{
			return TRUE;
		}
		return FALSE;
	...
	function procNproductAdminUpdateItemList()
	{
		...
		if(!in_array(Context::getRequestMethod(), array('XMLRPC', 'JSON')))
		{
			$returnUrl = Context::get('success_return_url') ? Context::get('success_return_url') : getNotEncodedUrl('', 'module', Context::get('module'), 'act', 'dispNproductAdminItemList', 'module_srl', Context::get('module_srl'), 'category', Context::get('category'), 's_item_name', Context::get('s_item_name'));
			$this->setRedirectUrl($returnUrl);
			return;
		}
	}
