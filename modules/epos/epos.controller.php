...
class eposController extends epos
{
	function processOrderInfo($strOrderInfo)
	{
		...
		if(!isset($orderInfo))
		{
			$this->stop('ORDERINFO is null');
		}

		if(!isset($orderInfo->ORDERNUMBER))
		{
	...
	function processResult($strRsXML)
	{
		...
		if(!isset($authInfo))
		{
			$this->stop('AUTHINFO is null');
		}
		// AUTHSTATUS
		if(!isset($authInfo->AUTHSTATUS))
		{
			$this->stop('AUTHSTATUS is null');
		}
		$authStatus = $authInfo->AUTHSTATUS->__toString();
		// AUTHCODE
		if(!isset($authInfo->AUTHCODE))
		{
			$this->stop('AUTHCODE is null');
		}
		$authCode = $authInfo->AUTHCODE->__toString();
		// AUTHMSG
		if(!isset($authInfo->AUTHMSG))
		{
			$this->stop('AUTHMSG is null');
		}
		$authMsg = $authInfo->AUTHMSG->__toString();
		// AUTHTIME
		if(!isset($authInfo->AUTHTIME))
		{
