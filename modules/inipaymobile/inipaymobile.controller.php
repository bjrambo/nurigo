...
class inipaymobileController extends inipaymobile
{
	...
	function processNextUrl($transaction_srl)
	{
		...
		if($post_output['P_STATUS'] == '00')
		{
			...
			if($transaction_info->payment_method == 'VA')
			{
				$output->add('state', '1');
			} // not completed
		}
	...
	function processNotiUrl()
	{
		...
		if($vars->P_TYPE == 'VBANK' && $vars->P_STATUS == '00')
		{
			return;
		}
		...
		if(($PGIP != "211.219.96.165" && $PGIP != "118.129.210.25" && $PGIP != "183.109.71.153" && $PGIP != "39.115.212.9") && !$_SESSION['inipaymobile_pass'])
		{
