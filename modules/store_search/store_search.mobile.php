<?php
	/**
	 * @class store_searchMobile
	 * @author CAMERON (mail@cameron.co.kr)
	 * @brief mobile class of the store_search module
	 **/

	require_once(_XE_PATH_.'modules/integration_search/integration_search.mobile.php');
	class store_searchMobile extends integration_searchMobile {
		...
		function IS() {
			...
			$this->setTemplatePath($this->module_path."/m.skins/".$config->skin."/");
			...
			if($is_keyword) {
				...
				switch($where) {
					case 'product' :
						...
						$output = $oIS->getProducts('include', $product_module_srl_list, $search_target, $is_keyword, $page, 12);
