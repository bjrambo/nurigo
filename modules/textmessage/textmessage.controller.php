...
class textmessageController extends textmessage
{
	 ...
	 *  $args->mode = 'test' // 모드값에 test를 넣으면 문자를 실제로 발송하지 않고 테스트요청만 가능함.
	...
	function sendMessage($args, $basecamp = FALSE)
	{
		...
		if($args->content)
		{
			$options->text = $args->content;
		}
		if($args->refname)
		{
			$options->refname = $args->refname;
		}
		if($args->country)
		{
		...
		if($args->reservdate)
		{
			$options->datetime = $args->reservdate;
		}
		if($args->route)
		{
		...
		if($args->app_version)
		{
			$options->app_version = $args->app_version;
		}
		if($args->sender_key)
		{
			$options->sender_key = $args->sender_key;
		}
		if($args->template_code)
		{
			$options->template_code = $args->template_code;
		}
		...
		if($result->group_id)
		{
			$output->add('group_id', $result->group_id);
		}
		return $output;
	}
