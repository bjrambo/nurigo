...
class epayView extends epay
{
	...
	function getPaymentForm($in_args)
	{
		...
		if($logged_info)
		{
			...
			if(!$in_args->purchaser_email)
			{
				$in_args->purchaser_email = $logged_info->email_address;
			}
			if(!$in_args->purchaser_telnum)
			{
		...
		else
		{
			...
			$payment_methods = array();
			if($module_info->pg1_module_srl)
			{
				$payment_methods = array_merge($payment_methods, $oEpayModel->getPaymentMethods($module_info->pg1_module_srl));
			}
			if($module_info->pg2_module_srl)
			{
				$payment_methods = array_merge($payment_methods, $oEpayModel->getPaymentMethods($module_info->pg2_module_srl));
			}
			if($module_info->pg3_module_srl)
			{
				$payment_methods = array_merge($payment_methods, $oEpayModel->getPaymentMethods($module_info->pg3_module_srl));
			}
			if($module_info->pg4_module_srl)
			{
				$payment_methods = array_merge($payment_methods, $oEpayModel->getPaymentMethods($module_info->pg4_module_srl));
			}
			Context::set('payment_methods', $payment_methods);
		}
	...
	function dispEpayTransaction()
	{
		...
		if(Context::get('n_page'))
		{
			...
			if($oEpayController->get('return_url'))
			{
				$this->setRedirectUrl($oEpayController->get('return_url'));
			}
		...
		if($logged_info)
		{
			...
			foreach($epay_user_info as $k => $v)
			{
				...
				if($v->state == 1)
				{
					$v->result_message = "결제진행중";
				}
				else if($v->state == 2)
				{
					$v->result_message = "결제성공";
				}
				else
				{
					$v->result_message = "결제실패";
				}

				if(is_array($v->order_title))
				{
					$v->order_title = implode($v->order_title, ',');
				}
				if(!$v->order_title)
				{
					$v->order_title = $v->target_module;
				}
