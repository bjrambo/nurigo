...
class store_reviewItem extends Object
{
	...
	function isGranted()
	{
		if($_SESSION['own_review'][$this->review_srl])
		{
			return true;
		}
	...
	{
		if($_SESSION['accessibled_review'][$this->review_srl])
		{
			return true;
		}
	...
	function getContent($add_popup_menu = true, $add_content_info = true, $add_xe_content_class = true)
	{
		...
		{
			$content = sprintf('%s<div class="review_popup_menu"><a href="#popup_menu_area" class="review_%d" onclick="return false">%s</a></div>', $content, $this->review_srl, Context::getLang('cmd_review_do'));
		}
		// if additional information which can access contents is set
		if($add_content_info)
		{
			$content = sprintf('<!--BeforeComment(%d,%d)--><div class="review_%d_%d xe_content">%s</div><!--AfterComment(%d,%d)-->', $this->review_srl, $this->get('member_srl'), $this->review_srl, $this->get('member_srl'), $content, $this->review_srl, $this->get('member_srl'));
...
/* End of file store_review.item.php */
/* Location: ./modules/store_review/store_review.item.php */
