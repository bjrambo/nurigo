...
class ncartController extends ncart
{
	...
	function procNcartInsertAddress()
	{
		...
		foreach($fieldset_list as $key => $fieldset)
		{
			foreach($fieldset->fields as $key2 => $field)
			{
				...
				{
					$title = $req_args->{$field->column_name};
				}
	...
	function triggerProcessReview(&$in_args)
	{
		...
		if($coupon_config->use_shop_coupon === 'yes')
		{
			...
			if($coupon_info !== false)
			{
				if($coupon_info->free_delivery == 'Y')
				{
					...
					$in_args->delivfee_inadvance = 'Y'; // 그게 아니라 배송비가 선결제가 되고 그 배송비를 빼야 하는게 맞음 by xianchoi
				}
				...
				if($coupon_info->discount_type == 'percent')
				{
					...
					$coupon_discount = $cart->total_discounted_price * ($coupon_info->discount / 100);
				}
		...
		$args->free_delivery = $coupon_info->free_delivery;	// 무배쿠폰여부 정보 추가
