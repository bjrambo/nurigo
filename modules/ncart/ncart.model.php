...
class ncartModel extends ncart
{
	...
	function getCartInfo($cartnos = null, $width = null, $height = null, $delivfee_inadvance = 'Y')
	{
		...
		else
		{
			...
			if($non_key)
			{
				$oNcartController->updateGuestCartItems($logged_info->member_srl, $non_key);
			}
	...
	*/
	...
	function getFavoriteItems($member_srl, $width = null, $height = null)
	{
		...
		if(!$width)
		{
			$width = $this->module_info->thumbnail_width;
		}
		if(!$height)
		{
			$height = $this->module_info->thumbnail_height;
		}
	...
	function getNcartCartItems()
	{
		...
		if($logged_info)
		{
			$mileage = $this->getMileage($logged_info->member_srl);
		}
	...
	function getInputHtml($formInfo)
	{
		...
		{
			$extentionReplace = array(
				'date' => zdate($extendForm->value, 'Y-m-d'),
...
EOD;
	...
	function triggerTransactionList(&$list)
	{
		...
		{
			if($val->target_module == 'ncart')
			{
				$list[$key]->target_act = 'getNcartAdminOrderDetails';
			}
		}
	}
}
